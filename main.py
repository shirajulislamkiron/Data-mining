...
class login_page:
    def __init__(self, root):
        ...
        label1 = Label(self.frame1, text="login", font=("times new roman", 40, "bold"), bg="yellow", fg="red").place(x=100,
                                                                                                                  y=300)

        label2 = Label(self.frame1, text="Details", font=("times new roman", 13, "bold"), bg="yellow",
    ...
    def login_func(self):
        if self.email_entry.get() == "" or self.password_entry.get() == "":
            ...
            try:
                connection = mysql.connector.connect(host="localhost", user="root", password="mysql7612",
                                             database="register1")
                cur = connection.cursor()
                cur.execute("select * from general where email=%s and password=%s",
                            (self.email_entry.get(), self.password_entry.get()))
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror("Error!", "Invalid USERNAME & PASSWORD", parent=self.window)
                else:
                    messagebox.showinfo("Success", "Wellcome to the login page", parent=self.window)
    ...
    def forgot_func(self):
        if self.email_entry.get() == "":
            ...
            try:
                connection = pymysql.connect(host="localhost", user="root", password="mysql7612",
                                             database="register1")
                cur = connection.cursor()
                cur.execute("select * from general where email=%s", self.email_entry.get())
                row = cur.fetchone()
                if row == None:
        ...
        if self.email_entry.get() == "" or self.sec_ques.get() == "Select" or self.new_pass.get() == "":
            ...
            try:
                connection = pymysql.connect(host="localhost", user="root", password="mysql7612",
                                             database="register1")
                cur = connection.cursor()
                cur.execute("select * from general where email=%s and question=%s and answer=%s",
                ...
                if row == None:
                    ...
                    try:
                        cur.execute("update general set password=%s where email=%s",
